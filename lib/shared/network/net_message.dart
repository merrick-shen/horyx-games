...
enum NetMessageType {
  ...
  /// 悔棋请求：一方向对方发起（棋类通用；房主转发给对方。
  /// 五子棋与象棋共用：悔棋语义一致——只能悔自己的上一手，
  /// 轮到对方落/走子时才可发起，载荷不含棋盘专属数据）
  undoRequest,

  /// 悔棋应答：对方同意/拒绝（棋类通用；房主转发回请求方）
  undoResponse,

  /// 悔棋生效：房主广播（双端各回退到指定手数快照）
  undoApplied,

  /// 认输：一方向房主声明（棋类通用；房主判定终局并广播）
  resign,

  // ============ 对局流程 · 中国象棋 ============

  /// 提交走子：客户端发给房主（仅轮到的玩家发送，
  /// 载荷 fromCol/fromRow/toCol/toRow 四个整数）
  moveSubmit,

  /// 走子校验结果：房主单独应答提交者（拒绝时携带原因，象棋）
  moveResult,

  /// 走子生效：房主广播（全端同步走子；
  /// 将死/困毙时携带胜方座位 winner 与终局原因 reason）
  moveApplied,
  ...
  /// 状态快照：房主 30Hz 广播（双方坦克/全部子弹/比分/战场阶段），
